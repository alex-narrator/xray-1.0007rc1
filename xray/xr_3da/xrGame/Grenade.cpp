                    ...
                    if(count_types>1)
                    {
                        ...
                        for(;it!=it_e;++it)
                        {
                            ...
                            if(pGrenade && !xr_strcmp(pGrenade->cNameSect(), sect_next_grn))
                            {
                                ...
                                //GRENADE_FROM_BELT
                                if(Belt())
                                    m_pCurrentInventory->Belt(this);                    // текущую гранату, обратно в по€с.
                                return true;
                            }
                        }
                    }
...
void CGrenade::GetBriefInfo(xr_string& str_name, xr_string& icon_sect_name, xr_string& str_count)
{
	...
	u32 ThisGrenadeCount	= m_pCurrentInventory->dwfGetGrenadeCount(*cNameSect(), true) +1; /*xer-urg: тут +1 т.к. обсчет кол-ва гранат дл€ показа на худе ведетс€ по кол-ву на по€се, а есть еще граната в слоте. 
	                                                                                          а написать расчет слот+по€с € не умею*/
	#else
