...
//вариации отображени€ и использовани€ слота гранаты - начало
...
#endif
//вариации отображени€ и использовани€ слота гранаты - конец
...
void CUIInventoryWnd::Update()
{
	...
	if(pEntityAlive) 
	{
		...
		if (pActor->inventory().m_slots[DETECTOR_SLOT].m_pIItem) //обнул€ем значение радиации дл€ прогрессбара в инвентаре если не экипирован детектор -- NO_RAD_UI_WITHOUT_DETECTOR_IN_SLOT
		{
