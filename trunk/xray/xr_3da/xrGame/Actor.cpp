...
//			mstate_real = mstate_wishful = NET_Last.mstate;
	...
	if(!input_external_handler_installed() && RQ.O &&  RQ.range<inventory().GetTakeDist()) 
	{
		...
		if (GameID() == GAME_SINGLE )
		{
			...
			{
				m_sDefaultObjAction = inventory().IsFreeHands() ? CStringTable().translate(m_pUsableObject->tip_text()) : m_sNoAnyAction;
			}
			else if (pEntityAlive)
			{
				...
				else if (!pEntityAlive->g_Alive())
				{
					if (b_allow_drag)
					{
						if (m_pMonsterWeLookingAt && inventory().IsFreeHands())	//на текущих конфигах нож - одноручный и с ним всегда IsFreeHands() == true, но вдруг будут двуручные ножи
						{       
							...
							m_sDefaultObjAction = inventory().IsFreeHands() ? m_sDeadCharacterUseOrDragAction : m_sNoAnyAction;
					}
					else
					{
...
//			conditions().ChangeRadiation(conditions().GetRadiationRestore	() * artefact->m_fRadiationRestoreSpeed * f_update_time * random_k * condition);
	...
	for (TIItemContainer::iterator it = inventory().m_all.begin(); inventory().m_all.end() != it; ++it)
	{
		...
		if (iitem != artefact || artefact->CanAffect())
			conditions().ChangeRadiation(conditions().GetRadiationRestore() * iitem->m_fRadiationRestoreSpeed * f_update_time * random_k * condition);
	}
