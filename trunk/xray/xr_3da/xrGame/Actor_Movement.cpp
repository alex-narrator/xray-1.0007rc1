...
void CActor::g_cl_ValidateMState(float dt, u32 mstate_wf)
{
	...
	if (this == Level().CurrentControlEntity())
	{
		...
		{
			if (g_eFreeHands == eFreeHandsManual) SetWeaponHideState(INV_STATE_LADDER, bOnClimbNow);
...
bool	CActor::CanJump				()
{
	bool can_Jump = /*!IsLimping() &&*/
		...
		&& !conditions().IsCantWalk();// && ((mstate_real&mcCrouch)==0); //если нет сил ходить то и прыгать нельзя
...
bool	CActor::CanMove				()
{
	...
	{
		if (mstate_wishful&mcAnyMove || mstate_wishful&mcJump) //сообщение о перегрузе также при попытке прыжка
		{
