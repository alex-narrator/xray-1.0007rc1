...
void CUIPdaWnd::Show()
{
	...
	if (pActor)
	{
		//if (psActorFlags.test(AF_FREE_HANDS)) pActor->SetWeaponHideState(INV_STATE_PDA_WND, true);  //спрячем оружие в руках
...
void CUIPdaWnd::Hide()
{
	...
	if (pActor)
	{
		//if (psActorFlags.test(AF_FREE_HANDS)) pActor->SetWeaponHideState(INV_STATE_PDA_WND, false);  //спрячем оружие в руках
