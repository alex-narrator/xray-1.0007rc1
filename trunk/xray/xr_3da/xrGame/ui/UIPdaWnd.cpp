...
void CUIPdaWnd::Show()
{
	...
	if (pActor)
	{
		if (psActorFlags.test(AF_HARD_INV_ACCESS)) pActor->SetWeaponHideState(INV_STATE_PDA_WND, true);  //спр€чем оружие в руках
	}
}
...
void CUIPdaWnd::Hide()
{
	...
	if (pActor)
	{
		if (psActorFlags.test(AF_HARD_INV_ACCESS)) pActor->SetWeaponHideState(INV_STATE_PDA_WND, false);  //спр€чем оружие в руках
	}
