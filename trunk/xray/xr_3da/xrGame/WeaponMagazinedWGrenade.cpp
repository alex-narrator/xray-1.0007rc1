...
CWeaponMagazinedWGrenade::CWeaponMagazinedWGrenade(LPCSTR name,ESoundTypes eSoundType) : CWeaponMagazined(name, eSoundType)
{
	...
	SetSlot(RIFLE_SLOT); // alpet: предполагаетс€ что пистолетов с такими аддонами не будет )
}
...
void  CWeaponMagazinedWGrenade::PerformSwitchGL()
{
	...
	if(m_bZoomEnabled && m_pHUD)
	{
		if (m_bGrenadeMode)
		{
			m_fZoomFactor = m_fIronSightZoomFactor; //зададим зум в режиме гранатомЄта
			LoadZoomOffset(*hud_sect, "grenade_");
		}
		else 
		{
			if (IsScopeAttached())
				m_fZoomFactor = m_fScopeZoomFactor; //если установлен прицел, то при выходе из режима гранатомЄта установим зум прицела
