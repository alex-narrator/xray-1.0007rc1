...
#define RADIATION_PARAMS_DEPENDECY     //задаёт пороговые значения радиации больше которых не восстанавливается здоровье, выносливость и снижается макимальный уровень здоровья (хит радиации здоровью лушче отключить)
#define NO_RAD_UI_WITHOUT_DETECTOR_IN_SLOT  //не отображать прогрессбар в инвентаре и иконку радиации на худе если в слоте нет детектора
#define LUMINOSITY_UI_INDICATOR        //использовать освещенность вместо заметности на худовой шкале
#define MINIMAP_ON_TAB              //отображение миникарты по нажатию кнопки TAB